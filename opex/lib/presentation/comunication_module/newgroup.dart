...
class _NewGroupState extends State<NewGroup> {
  bool isSelect = false;
  List<UserModelInventory> userIdList = [];
  List<UserModelInventory> selectedUserIdList = [];
                        ...
                        child: ListView.separated(
                          ...
                          itemBuilder: (context, index) => NewGroupCard(
                              ...
                              onPress: (val) {
                                ...
                                setState(() {});
                              },photo: state.registeresUsers[index].photo,
                              // userId: state.registeresUsers[index].id,
                              name: state.registeresUsers[index].name,
                              lastName: state.registeresUsers[index].lastName,
                              email: state.registeresUsers[index].mail),
...
class NewGroupCard extends StatefulWidget {
  ...
  final String? userId;
  List<UserModelInventory> userIdList = [];
  final Function(List<UserModelInventory>) onPress;
  NewGroupCard(
      {Key? key,
      this.email,
      this.name,
  ...
  Widget build(BuildContext context) {
    return InkWell(
      onTap: () {
        ...
        // print(widget.userId);
        if (isSelect == true) {
          widget.userIdList.add(UserModelInventory(
              name: widget.name,
              lastName: widget.lastName,
              mail: widget.email,
              photo: widget.photo));
        } else {
          widget.userIdList.remove(UserModelInventory(
              name: widget.name,
              lastName: widget.lastName,
              mail: widget.email,
              photo: widget.photo));
      ...
      child: Container(
          ...
          child: Row(
            ...
            children: [
              ...
              isSelect
                  ? SvgPicture.string(CommunicationSvg().tickIcon)
                  : Container()
            ],
          )),
    );
