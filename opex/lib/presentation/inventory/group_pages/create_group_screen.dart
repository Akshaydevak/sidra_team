...
class CreateGroupScreen extends StatefulWidget {
  final bool? edit;
  const CreateGroupScreen({Key? key, this.edit}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    return MultiBlocListener(
      listeners: [
          ...
          listener: (context, state) {
            if (state is CreateGroupLoading) {
              showSnackBar(context,
                  message: "Group Creation Loading",
        ...
        BlocListener<InventoryBloc, InventoryState>(
          listener: (context, state) {
            if (state is GroupReadLoading) {

            }
            else if (state is GroupReadSuccess) {
              modelData=state.divisionModel;
              nameController.text=modelData?.name??"";
              altNameController.text=modelData?.displayName??"";
              ...
              divisionCode=modelData?.catCode??"";
              isActive=modelData?.isActive??false;
              setState(() {

              });

            } else if (state is GroupReadFailed) {
              showSnackBar(context,
                  message: state.error, color: ColorPalette.black);
            }
          },
        ...
        BlocListener<InventoryBloc, InventoryState>(
          listener: (context, state) {
            if (state is GroupUpdateLoading) {
              showSnackBar(context,
                  message: "Group Updation Loading",
                  color: ColorPalette.green);
              ...
              showSnackBar(context,
                  ...
                  color: ColorPalette.green);
              Navigator.pop(context);
            } else if (state is GroupUpdateFailed) {
              showSnackBar(context,
                  message: state.error, color: ColorPalette.black);
            }
          },
      ...
      child: Scaffold(
        ...
        appBar: PreferredSize(
          ...
          child: BackAppBar(
            label: widget.edit==true?"Update Group":"Create Group", isAction: false,
                ...
                children: [
                  ...
                  widget.edit==true?GradientButton(
                      onPressed: () {
                        ...
                        context.read<InventoryBloc>().add(UpdateGroupEvent(
                            image: imageId,
                            isActive: isActive,
                            id: modelData?.id??0,
                            displayName: altNameController.text,
                      ...
                      onPressed: () {
                        ...
                        context.read<InventoryBloc>().add(GroupCreateEvent(
                            image: imageId,
                            displayName: altNameController.text,
                            categoryCode: divisionCode??"",
                            searchName: searchNameController.text,
                          ...
                          if (state is GetCategoryListSuccess) {
                            ...
                            return Container(
                              ...
                              child: Column(
                                ...
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  divisionCode==""?Container():GestureDetector(
                                    onTap: () {
                                      divisionCode="";
                                      refresh();
                                      setState(() {});
