...
class InventoryCreateScreen extends StatefulWidget {
  ...
  const InventoryCreateScreen({Key? key, required this.edit}) : super(key: key);

  @override
  State<InventoryCreateScreen> createState() => _CreateScreenState();
}

class _CreateScreenState extends State<InventoryCreateScreen> {
  bool isActive = true;
  ...
  Widget build(BuildContext context) {
    ...
    return MultiBlocListener(
      listeners: [
        ...
        BlocListener<InventoryBloc, InventoryState>(
          listener: (context, state) {
            if (state is CreateDivisionLoading) {
              showSnackBar(context,
                  message: "Division Creation Loading",
                  color: ColorPalette.green);
            } else if (state is CreateDivisionSuccess) {
              showSnackBar(context,
                  message: state.successMessage??"",
                  color: ColorPalette.green);
              context.read<InventoryBloc>().add(GetDivisionListEvent('','',''));
              Navigator.pop(context);
            } else if (state is CreateDivisionFailed) {
              showSnackBar(context,
        ...
        BlocListener<InventoryBloc, InventoryState>(
          listener: (context, state) {
            if (state is DivisionReadLoading) {

            } else if (state is DivisionReadSuccess) {

              modelData=state.divisionModel;
              print("mod${modelData?.image}");
            ...
            } else if (state is DivisionReadFailed) {
              showSnackBar(context,
                  message: state.error, color: ColorPalette.black);
            }
          },
        ...
        BlocListener<InventoryBloc, InventoryState>(
          listener: (context, state) {
            if (state is DivisionUpdateLoading) {
              showSnackBar(context,
                  message: "Division Updation Loading",
                  color: ColorPalette.green);
            } else if (state is DivisionUpdateSuccess) {
              ...
              context.read<InventoryBloc>().add(GetDivisionListEvent('','',''));
              Navigator.pop(context);
            } else if (state is DivisionUpdateFailed) {
              showSnackBar(context,
                  message: state.error, color: ColorPalette.black);
            }
          },
      ...
      child: Scaffold(
        ...
        appBar: PreferredSize(
          ...
          child: BackAppBar(
            label: widget.edit==true?"Update Division":"Create Division", isAction: false,
            onTap: (){
              Navigator.pop(context);
            },
          ),
                ...
                children: [
                  ...
                  widget.edit==true?GradientButton(
                      onPressed: () {
                        ...
                        context.read<InventoryBloc>().add(UpdateDivisionEvent(
                            id: modelData?.id??0,
                            isActive: isActive,
                            image: imageId,
                      ...
                      onPressed: () {
                        ...
                        context.read<InventoryBloc>().add(DivisionCreateEvent(
                            image: imageId,
                            displayName: displayNameController.text,
                            searchName: searchNameController.text,
                            name: nameController.text,
