...
class InventoryStockPartitionScreen extends StatefulWidget {
  // final Function(String)? onChangeItem;
  final Function(int,String)? onChangeStock;
  // final Function(int)? onChangeVariantId;
  // final Function(BaseUomData?)? onChangeBaseUom;
  // int? groupId;

  InventoryStockPartitionScreen(
      {Key? key,
        this.onChangeStock
      })
      : super(key: key);
  ...
  @override
  State<InventoryStockPartitionScreen> createState() =>
      _InventoryStockPartitionScreenState();
}

class _InventoryStockPartitionScreenState extends State<InventoryStockPartitionScreen> {
  int? select = Variable.stock;
  ...
  @override
  void initState() {
    context.read<InventoryBloc>().add(GetStockPartitionEvent(id: authentication.authenticatedUser.organisationCode));
    super.initState();
  }
    ...
    return BlocListener<InventoryBloc, InventoryState>(
        listener: (context, state) {
          ...
          if (state is GetStockSuccess) {
            ...
            // print("state${item[0].name}");
          }
          if (state is GetStockFailed) {
            setState(() {
              isFailed = true;
            });
          }
        ...
        child: Column(children: [
          ...
          SearchCard(
            onchange: (String val) {
              ...
              context.read<InventoryBloc>().add(GetStockPartitionEvent(id: authentication.authenticatedUser.organisationCode??"",search: val));
            },
          ),
          SizedBox(
            height: 5,
                    ...
                    itemBuilder: (context, index) => GestureDetector(
                        child: InventoryCatProductCard(
                          onTap: () {
                            ...
                            setState(() {
                              Variable.item=index;
                              widget.onChangeStock!(item[index].id??0,item[index].name ?? "");
