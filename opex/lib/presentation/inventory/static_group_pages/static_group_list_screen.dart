...
import 'create_static_group_screen.dart';



class StaticGroupListScreen extends StatefulWidget {
  const StaticGroupListScreen({Key? key}) : super(key: key);

  @override
  State<StaticGroupListScreen> createState() => _StaticGroupListScreenState();
}

class _StaticGroupListScreenState extends State<StaticGroupListScreen> {
  ...
  @override
  void initState() {
    context.read<InventoryBloc>().add(const GetStaticGroupListEvent('','',''));
  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<InventoryBloc, InventoryState>(
      listener: (context, state) {
          ...
          if(state is DeleteStaticGroupSuccess){
            showSnackBar(context,
                message: "Static Group Successfully Deleted",
                color: ColorPalette.green);
            context.read<InventoryBloc>().add(const GetStaticGroupListEvent('','',''));
          }
          if(state is DeleteStaticGroupFailed){
            showSnackBar(context,
            ...
            action: GestureDetector(
              onTap: () {
                PersistentNavBarNavigator.pushNewScreen(
                  context,
                  screen: CreateStaticGroupScreen(edit: false,),
                  withNavBar: true,
                ...
                child: Column(
                  ...
                  children: [
                    SearchCard(
                      ...
                      onchange: (jj){
                        context.read<InventoryBloc>().add( GetStaticGroupListEvent(jj??"",'',''));
                        setState(() {
  ...
  builder: (context, state) {
                            ...
                            itemBuilder: (context, index) =>
                                GestureDetector(
                                    child: InventoryDivisionCard(
                                      onTapAll: (){
                                        context.read<InventoryBloc>().add(StaticGroupReadEvent(division[index].id??0));
                                        PersistentNavBarNavigator.pushNewScreen(
                                          context,
                                          screen: CreateStaticGroupScreen(edit: true),
                                          withNavBar: true,
                                          // OPTIONAL VALUE. True by default.
                                          pageTransitionAnimation:
                                          PageTransitionAnimation.fade,
                                                  ...
                                                  children: <Widget>[
                                                    ...
                                                    Container(
                                                      ...
                                                      child:Column(
                                                        ...
                                                        crossAxisAlignment: CrossAxisAlignment.start,
                                                        children: [
                                                          Text("Are You Sure To Delete The Static Group..?",
                                                            style: GoogleFonts.roboto(
                                                                fontSize: w/24,
                                                          ...
                                                          GradientButton(
                                                              color: ColorPalette.primary,
                                                              onPressed: () {
                                                                context.read<InventoryBloc>().add(DeleteStaticGroupEvent(division[index].id??0));
                                                                Navigator.pop(context);
                                                              },
    ...
    if (state is GetStaticGroupListFailed) {
      // group.clear();
      division.clear();
      // setState(() {});
